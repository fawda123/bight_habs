...
```{r, out.height = "100%", fig.height = 9, fig.width = 7}
...
pcamod <- PCA(tomodall, scale.unit = F, graph = F)

p1 <- ggord(pcamod, vec_ext = 3.5, alpha = 0.5, size = 1, txt = 4, arrow = 0.2, repel = T, coord_fix = F, labcol = 'tomato1', veccol = 'tomato1') + 
  mythm2 +
  ggtitle('All dates')
...
pcamodgrp <- tsdat %>% 
  ...
  mutate(
    plo = purrr::pmap(list(qrt, data), function(qrt, data){
      ...
      pcamod <- PCA(tomod, scale.unit = F, graph = F)

      p <- ggord(pcamod, vec_ext = 3.5, alpha = 0.5, size = 1, txt = 4, arrow = 0.2, repel = T, coord_fix = F, labcol = 'tomato1', veccol = 'tomato1') + 
        mythm2 +
        ggtitle(qrt)
...
grid.arrange(
  p1, 
  arrangeGrob(pcamodgrp[[3]], pcamodgrp[[4]], pcamodgrp[[1]], pcamodgrp[[2]], ncol = 2), 
  ncol = 1
)
...
#     ) %>% 
#   select(ammonia, chla, da, nitrate, nitrite, phosphorus, pntot, silicate, temp) %>% 
#   mutate_all(function(x) ifelse(is.na(x), median(x, na.rm = T), x)) #%>%
#   # as.data.frame %>% 
#   # imputePCA %>% 
#   # .$completeObs
#   
# nmsmod <- metaMDS(tomodall, distance = 'euclidean')
# save(nmsmod, file = here('data', 'nmsmod.RData'), compress = 'xz')
# data(nmsmod)
# p1 <- ggord(nmsmod, vec_ext = 1, alpha = 0.5, size = 1, txt = 4, arrow = 0.2, repel = T, coord_fix = F, labcol = 'tomato1', veccol = 'tomato1') + 
#   mythm2 +
#   ggtitle('All dates')
# 
# pcamodgrp <- tsdat %>% 
