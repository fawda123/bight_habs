...
tomod <- tsdat %>%
  filter(location %in% 'Newport Pier') %>% 
  select(tempanom, upwell, din, phosphorus, ntop, siton) %>%
  rename(
    `N:P` = ntop,
    `Si:N` = siton,
...
plos <- mrgs$frm
grid.arrange(
  arrangeGrob(plosall[[1]], plos[[4]], plos[[1]], plos[[2]], plos[[3]], ncol = 1),
  arrangeGrob(plosall[[2]], plos[[8]], plos[[5]], plos[[6]], plos[[7]], ncol = 1),
  arrangeGrob(plosall[[3]], plos[[12]], plos[[9]], plos[[10]], plos[[11]], ncol = 1),
  ncol = 3
)
